...
# Objects only (no dq_reports, no DB)
...
def _df_to_records(df) -> List[Dict[str, Any]]:
    return [] if df is None else getattr(df, "to_dict", lambda **_: [])(orient="records")
...
def _get_records(obj_cls: type, report_date: Optional[date], limit: int) -> List[Dict[str, Any]]:
    fn = getattr(obj_cls, "get_dataframe", None)
    if not callable(fn): return []
    common = dict(limit=limit, pyspark=False)
    sig = inspect.signature(fn)

    # Prefer calling with a date kwarg if the object supports one
    if report_date:
        for kw in DATE_KWS:
    ...
    # Otherwise fetch and filter in-memory (in case the object has no date kw)
    try:
        recs = _df_to_records(fn(**common))
        return recs if not report_date else [r for r in recs if _parse_date_any(r.get("report_date")) == report_date]
    except:  # noqa: E722
        return []


def _latest_report_date_objects(limit_probe: int = 1000) -> Optional[date]:
    latest: Optional[date] = None
    for _, cls in SECTIONS:
        for r in _get_records(cls, None, limit_probe):
...
def dq_combined(request, report_date: Optional[date] = None, limit: int = 500):
    # Default to most recent date across the six objects
    if report_date is None:
        report_date = _latest_report_date_objects()

    all_rows: List[Dict[str, Any]] = []
    for name, cls in SECTIONS:
        recs = _get_records(cls, report_date, limit)
        for r in recs:
