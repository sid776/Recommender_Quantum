...
// --- helpers to normalize and pick max date ---
...
function maxDateStr(dates) {
  ...
  for (const d of dates) {
    ...
    if (!norm) continue;
    if (!max || norm > max) max = norm; // lexicographic works for YYYY-MM-DD
  }
  return max;
}
// ----------------------------------------------
...
export default function CosmosReports() {
  ...
  // *** Minimal change: on first load, fetch without date, pick MAX(report_date), then fetch that date ***
  useEffect(() => {
    if (fetchedOnce.current) return;
    fetchedOnce.current = true;

    (async () => {
      // Step 1: get a superset (no date) to discover the latest date
      const res = await fetch(`${API_ENDPOINT}?limit=5000`, { headers: { Accept: "application/json" } });
      let payload = [];
      if (res.ok) {
        const j = await res.json().catch(() => []);
        payload = Array.isArray(j) ? j : Array.isArray(j?.rows) ? j.rows : [];
      }

      // Step 2: compute max report_date across payload
      const latest = maxDateStr((payload || []).map((r) => r?.report_date).filter(Boolean));

      // Step 3: if we found one, set the COB and fetch that exact date
      if (latest) {
        setValue("report_date", latest, { shouldDirty: false });
        await fetchData(latest);
      } else {
        // fallback: just show whatever came back
        setRows(payload || []);
      }
    })();
  }, [setValue]);
  ...
  const onGridReady = (params) => {
    ...
    params.api.closeToolPanel(); // keep buttons, panel closed
  };
