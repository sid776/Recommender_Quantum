...
function buildColumnDefs(rows) {
  ...
  // Pull out the Risk Factor Id col so we can place it right after Report Date
  let riskFactorCol = null;
  const remainingPreferred = [];
  for (const c of preferredCols) {
    if (c.headerName === "Risk Factor Id" && !riskFactorCol) riskFactorCol = c;
    else remainingPreferred.push(c);
  }
  ...
  // Final order: groups → Report Date → Risk Factor Id (if found) → Years → other preferred → extras
  return [
    ...groupCols,
    reportDateCol,
    ...(riskFactorCol ? [riskFactorCol] : []),
    ...yearCols,
    ...remainingPreferred,
    ...extras,
  ];
