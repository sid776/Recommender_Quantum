...
// Alias to satisfy “use AggridTable element” without changing behavior
...
// tiny helper to build URL (matches “use existing function / no conversion needed” feedback)
const buildUrl = (dateStr) =>
  dateStr ? `${API_ENDPOINT}?report_date=${dateStr}&limit=500` : `${API_ENDPOINT}?limit=500`;
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    setLoading(true);
    try {
      const res = await fetch(buildUrl(dateStr), { headers: { Accept: "application/json" } });
      if (!res.ok) {
        setRows([]);
        return;
      }
      ...
      // default COB to the latest date present in payload when user hasn't picked one
