...
/* ---- robust date helpers (choose latest present in payload) ---- */
...
function maxDateStr(dates) {
  ...
  for (const d of dates) {
    ...
    if (!norm) continue;
    if (!max || norm > max) max = norm; // YYYY-MM-DD lexical compare
  }
  return max;
}
/* ---------------------------------------------------------------- */
...
/* ---- parse any backend shape into a flat array ---- */
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // If user didn't pick a date, lock to the latest present in payload (works for object-only API, no hacks)
  ...
  useEffect(() => {
    ...
    // When report_date is set by the first fetch (latest), refetch for that exact day
