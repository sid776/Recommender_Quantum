...
// choose a “report_date-ish” for a row
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // If user selected a date, guarantee report_date is present per row (so the column shows it)
      if (dateStr) {
        ...
        setRows(data || []);
        // First load: default COB to the most recent across report_date/as_of_date/as_of_dt
        if (data?.length) {
          const candidates = data
            .map((r) => deriveRowDate(r))
            .filter(Boolean);
