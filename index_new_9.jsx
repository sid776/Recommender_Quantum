...
function normalizeRows(rows) {
  ...
  return (rows || []).map((r) => {
    ...
    // normalize report_date "YYYYMMDD" -> "YYYY-MM-DD" for display consistency
  ...
  return keys
    ...
    .map((k) => {
      const col = {
        ...
        headerValueGetter: () => prettify(k), // keep header as plain text (no "Sum(...)")
        field: k,
        sortable: true,
        filter: true,
        resizable: true,
...
export default function CosmosReports() {
  ...
  // 1) On mount: get latest date once and fill the date field
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
  ...
  // 2) Auto-run whenever the date changes
      ...
      <FormProvider {...methods}>
        <form className="flex flex-col gap-3">
          <div className="p-4 bg-white shadow-md rounded-lg" style={{ marginTop: -16 }}>
            ...
            <div className="pt-2">
              <div className="text-sm font-semibold mb-2">Combined DQ Reports{hasData ? `  ${normRows.length}` : ""}</div>

              {loading ? (
                <Skeleton height="520px" rounded="md" />
              ) : hasData ? (
                <Box className="ag-theme-alpine rounded-lg shadow-md" style={{ height: "600px", width: "100%" }}>
                  <AgGridReact
                    rowData={normRows}
