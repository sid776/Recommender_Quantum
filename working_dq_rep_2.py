...
def _date_variants(d: date) -> List[Union[date, str]]:
    ymd_dash = d.strftime("%Y-%m-%d")
    ymd = d.strftime("%Y%m%d")
    return [d, ymd_dash, ymd]
...
def _call_object_get_dataframe(obj_cls: type, report_date: Optional[date], limit: int) -> List[_ReportRow]:
    ...
    def _common(order_dir: str, lim: int):
    ...
    for order_dir in ("desc", "asc"):
        for lim in (limit, max(limit, 2000), max(limit, 10000), max(limit, 50000)):
            try:
...
def _collect_sample_rows(limit_per: int = 500) -> List[_ReportRow]:
...
def _resolve_latest_report_date_via_objects() -> Optional[date]:
    rows = _collect_sample_rows(limit_per=1000)
    if not rows:
        return None
