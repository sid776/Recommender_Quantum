...
/** ---- SMART KEY MATCHING (fixes Databricks prefix like "1.2 std_value") ---- **/
const norm = (s) => String(s || "").toLowerCase().replace(/[^a-z0-9_]/g, "");
function keyMatches(actualKey, candidate) {
  const a = norm(actualKey);
  const c = norm(candidate);
  // exact OR ends-with (so "12std_value" matches "std_value")
  return a === c || a.endsWith(c);
}
function pickKey(row, candidates) {
  if (!row) return null;
  const keys = Object.keys(row);
  // Prefer exact/ends-with matches; if multiple, take the one with longest candidate (most specific)
  let best = null;
  let bestLen = -1;
  for (const k of keys) {
