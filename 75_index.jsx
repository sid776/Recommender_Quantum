...
/* Canonicalize for robust fuzzy matching */
...
/* key matching: canonical equality or suffix match (tolerates prefixes like "1.2 ") */
function _matchKey(actualKey, candidatesCanonSet, candidatesCanonArr) {
  const a = canon(actualKey);
  if (candidatesCanonSet.has(a)) return true;
  for (const c of candidatesCanonArr) if (a.endsWith(c)) return true;
  return false;
}

function pickKey(row, candidates) {
  if (!row) return null;
  const candArr = (candidates || []).map(canon);
  const candSet = new Set(candArr);
  for (const k of Object.keys(row)) {
    if (_matchKey(k, candSet, candArr)) return k;
  }
  return null;
}
...
function findKeyInRows(rows, candidates) {
  ...
  for (const r of rows) {
    for (const k of Object.keys(r || {})) {
      if (_matchKey(k, candSet, candArr)) tally.set(k, (tally.get(k) || 0) + 1);
    }
  }
  let best = null, max = 0;
...
/* normalize rows (numbers, booleans, year columns) — do not blank out preferred fields */
function normalizeRows(rows) {
  return (rows || []).map((r) => {
    const o = { ...(r || {}) };

    // Map Y2021..Y2025 → 2021..2025 if present; otherwise keep existing 2021..2025 or set 0 only for year columns.
    FIXED_YEARS.forEach((y) => {
      ...
      else if (!(y in o)) o[y] = 0;
      if (`Y${y}` in o) delete o[`Y${y}`];
    });

    // hidden book grouping
    const bKey = pickKey(o, ["book", "book_nm", "book name"]);
    o.__book = bKey ? o[bKey] : "";

    // backfill stale/as_of date from report_date if missing
    ...
    // numeric coercion ONLY if value exists (don’t turn blanks into 0)
    ["mean_value", "z_score", "std_value", "risk_factor_value"].forEach((nm) => {
      const k = pickKey(o, [nm]);
      if (k && o[k] !== undefined && o[k] !== null && o[k] !== "") {
        const n = Number(o[k]);
...
/* build columns: years first, then preferred, then extras — with per-row valueGetter for preferred */
function buildColumnDefs(rows) {
  ...
  const numericCanon = new Set(["zscore","stdvalue","meanvalue","riskfactorvalue"]);

  const preferredCols = preferredKeyMap.map(({ header, actualKey, keys }) => {
    const isNumeric = keys.some((k) => numericCanon.has(canon(k)));
    // ALWAYS resolve per row so rows with different key variants still populate
    return {
      headerName: header,
      sortable: true,
      resizable: true,
      ...
      valueGetter: (p) => {
        ...
        // prefer the common key if present on this row
        if (actualKey && p.data.hasOwnProperty(actualKey)) return p.data[actualKey];
