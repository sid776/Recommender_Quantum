...
/* <<< KEY MATCHING FIX >>> 
   - direct canonical equality
   - OR the actual key can *end with* the candidate (so "1.2 std_value" matches "std_value") */
function _matchKey(actualKey, candidatesCanonSet, candidatesCanonArr) {
  ...
  for (const c of candidatesCanonArr) {
    if (a.endsWith(c)) return true; // suffix match to tolerate prefixes like "1.2 "
  }
  return false;
}
...
function findKeyInRows(rows, candidates) {
  ...
  for (const r of rows) {
    for (const k of Object.keys(r || {})) {
      if (_matchKey(k, candSet, candArr)) {
...
/* normalize rows (numbers, booleans, year columns) — DO NOT zero-out preferred fields */
...
function buildColumnDefs(rows) {
  ...
  const preferredCols = preferredKeyMap.map(({ header, actualKey, keys }) => {
    ...
    if (actualKey) {
      ...
      // fallback: resolve per-row (picks keys like "1.2 std_value")
      col.valueGetter = (p) => {
