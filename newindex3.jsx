...
const ENDPOINT = "/dq/combined"; // useFetch will prepend /api
...
function buildHierarchy(rows) {
  if (!rows?.length) return { map: {}, ruleCounts: {}, bookCounts: {}, hasRule: false, hasBook: false };
  ...
  rows.forEach((r) => {
    ...
    if (!map[rt]) map[rt] = {};
    map[rt][bk] = true;
    ruleCounts[rt] = (ruleCounts[rt] || 0) + 1;
    bookCounts[`${rt}||${bk}`] = (bookCounts[`${rt}||${bk}`] || 0) + 1;
  });

  return { map, ruleCounts, bookCounts, hasRule, hasBook };
}
...
export default function CosmosReports() {
  ...
  const [openByRule, setOpenByRule] = useState({});
  const [activeRule, setActiveRule] = useState(null);
  const [activeBook, setActiveBook] = useState(null);

  const { map, ruleCounts, bookCounts, hasRule, hasBook } = useMemo(() => buildHierarchy(rows), [rows]);
                  ...
                  <Collapsible.Content>
                    <div className="flex flex-col gap-2">
                      {Object.keys(map)
                        .sort()
                        .map((rt) => {
                          const open = !!openByRule[rt];
                          const books = Object.keys(map[rt] || {}).sort();
                          const count = ruleCounts[rt] || 0;
                          return (
                            <div key={rt} className="border rounded-md bg-gray-50">
                              <div className="flex items-center justify-between px-3 py-2">
                                <div
                                  className="flex items-center gap-2 cursor-pointer"
                                  onClick={() => setOpenByRule((s) => ({ ...s, [rt]: !open }))}
                                >
                                  <i className={`ph ${open ? "ph-caret-down" : "ph-caret-right"}`} />
                                  <span
                                    className={`${String(activeRule) === rt && !activeBook ? "font-semibold" : ""}`}
                                    onClick={(e) => {
                                      e.stopPropagation();
                              ...
                              {open && (
                                <div className="flex flex-col pb-2">
                                  {books.map((bk) => {
                                    const bCount = bookCounts[`${rt}||${bk}`] || 0;
                                    const selected = String(activeRule) === rt && String(activeBook) === bk;
                                    return (
                                      <div
                                        key={`${rt}||${bk}`}
                                        className="px-8 py-1.5 hover:bg-gray-100 cursor-pointer"
                                        onClick={() => setActiveBook((prev) => (selected ? null : bk))}
                                      >
                                        <span className={`text-sm ${selected ? "font-semibold" : ""}`}>{bk}</span>
                                        <span className="ml-2 text-xs text-gray-600">{bCount}</span>
                                      </div>
                                    );
                                  })}
                                </div>
