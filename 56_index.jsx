...
function getYearKeys(rows) {
  ...
  const years = Array.from(
    new Set(rows.flatMap((r) => Object.keys(r || {}).filter((k) => /^\d{4}$/.test(k) && Number(k) >= MIN_YEAR)))
  ).sort();
...
function maxDateStr(dates) {
  ...
  for (const d of dates) {
    ...
    if (!norm) continue;
    if (!max || norm > max) max = norm; // lexical compare for YYYY-MM-DD
  }
  return max;
}
...
function buildColumnDefs(rows, YEARS) {
  // include report_type so we can group by it
  const base = ["report_type", "report_date", "risk_factor_id", "rule_type", "book"];
  ...
  const extras = allRowKeys.filter((k) => {
    ...
    if (/^Y\d{4}$/.test(k)) return false; // drop Y2021...Y2025 duplicates
  ...
  return allKeys.map((k) => {
    ...
    const isLikelyNumber =
      isYear ||
      ["z score", "std value", "mean value", "risk factor value"].includes((headerName || "").toLowerCase()) ||
      /^\d{4}$/.test(k);
    const isLikelyDate = (headerName || "").toLowerCase().includes("date") || k.toLowerCase().endsWith("_dt");
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // choose the latest report_date present in the payload (not the first row)
