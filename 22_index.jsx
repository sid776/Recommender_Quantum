...
/* ---- robust date parsing for selecting latest ---- */
      ...
      if ((!dateStr || !dateStr.length) && data?.length) {
        ...
        for (const r of data) {
          for (const k of Object.keys(r || {})) {
            const lk = lc(k);
            if (lk.includes("date") || lk.endsWith("_dt") || lk === "cob_date") {
              allDates.push(r[k]);
            }
          }
        }
        ...
        if (latest) {
          ...
          await fetchData(latest); // immediately show that day's slice
          return;
        }
      }
    } finally {
