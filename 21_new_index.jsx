...
function maxDateStr(dates) {
  ...
  for (const d of dates) {
    ...
    if (!norm) continue;
    if (!max || norm > max) max = norm; // lexical compare is fine for YYYY-MM-DD
  }
  return max;
}
// ------------------------------------------------
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // MINIMAL CHANGE: pick latest date across report_date / as_of_date / as_of_dt / cob_date
      if ((!dateStr || !dateStr.length) && data?.length) {
        const keys = ["report_date", "as_of_date", "as_of_dt", "cob_date"];
        const allDates = [];
        for (const r of data) for (const k of keys) if (r && r[k] != null) allDates.push(r[k]);
