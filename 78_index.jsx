...
// turn “1.2_std_value” → “12stdvalue” for matching, keep only a–z0–9
...
const PREFERRED_FIELDS = [
  ...
  // Stale date and As Of Date shown separately (can resolve from same underlying column)
  ...
  // numeric fields – include “1.2_std_value”, “1.2_mean_value”
...
// helpers to resolve keys
function pickKey(row, candidates) {
  if (!row) return null;
  const candSet = new Set((candidates || []).map(normKey));
  for (const k of Object.keys(row)) if (candSet.has(normKey(k))) return k;
  return null;
}
function findKeyInRows(rows, candidates) {
  if (!rows?.length) return null;
  const candSet = new Set((candidates || []).map(normKey));
  // prefer the key that appears most often
  const tally = new Map();
  for (const r of rows) {
    for (const k of Object.keys(r || {})) {
      if (candSet.has(normKey(k))) tally.set(k, (tally.get(k) || 0) + 1);
    }
  }
  let best = null, max = 0;
...
/* normalize rows (years, booleans; do NOT force zeros for preferred numerics) */
function normalizeRows(rows) {
  return (rows || []).map((r) => {
    const o = { ...(r || {}) };

    // Map Y2021..Y2025 → 2021..2025. Default 0 only for the year columns.
    ...
    // fill a date-ish field from report_date if an expected date field is empty
    ...
    // keep std/mean/z as real numbers if present; leave null if absent (no fake zeros)
    const numericNames = [
...
export default function CosmosReports() {
  ...
  // Keep side buttons visible on the right, closed by default
  ...
  function doExport() {
    ...
    // get leaf rows currently shown (respecting groups/filters/sorts)
    const rowsOut = [];
    api.forEachNodeAfterFilterAndSort((n) => {
      if (!n.group && n.data) rowsOut.push(n.data);
    });

    // visible, non-group columns (skip hidden group cols)
