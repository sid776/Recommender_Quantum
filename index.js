      ...
      <table style={{width:'100%', borderCollapse:'collapse'}}>
        <thead>
          <tr>
            {columns.map(c => (
              <th key={c.key}
                  style={{textAlign:'left', padding:'8px', fontSize:13, color:'#6b7280', borderBottom:'1px solid #e5e7eb'}}>
                {c.label}
              </th>
            ))}
          </tr>
...
export default function DataStatus() {
  ...
  return (
    <div style={{padding:20, background:'#f9fafb', minHeight:'100vh'}}>
      ...
      <div style={{color:'#6b7280', marginBottom:16}}>Latest COB snapshots across datasets</div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))', gap:16}}>
        ...
        <Box title="Valuation: Run Count">
          ...
          <div style={{color:'#6b7280', marginTop:4}}>Distinct RUN_ID for latest COB</div>
        </Box>
      </div>
    </div>
  );
