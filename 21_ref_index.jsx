...
// --- date helpers to find latest report_date ---
function parseYMD(anyVal) {
  if (anyVal == null) return null;
  // Date object
  if (anyVal instanceof Date && !isNaN(anyVal.getTime())) {
    return anyVal.toISOString().slice(0, 10);
  }
  const s = String(anyVal).trim().replace(/['"]/g, "");
  // ISO datetime like 2025-09-19T00:00:00 or with space
  const iso10 = s.slice(0, 10);
  if (/^\d{4}-\d{2}-\d{2}/.test(s) && /^\d{4}-\d{2}-\d{2}$/.test(iso10)) return iso10;
  // YYYY-MM-DD
...
function maxDateStr(dates) {
  ...
  for (const d of dates) {
    ...
    if (!norm) continue;
    if (!max || norm > max) max = norm; // lexical compare works for YYYY-MM-DD
  }
  return max;
}
// ------------------------------------------------
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // minimal change: compute latest date across all likely date fields; refetch with that
      if ((!dateStr || !dateStr.length) && data?.length) {
        const knownKeys = ["report_date", "as_of_date", "as_of_dt", "cob_date"];
        const allDates = [];
        for (const r of data) {
          const keys = Object.keys(r || {});
          for (const k of keys) {
            const lk = lc(k);
            if (knownKeys.includes(lk) || lk.includes("date") || lk.endsWith("_dt")) {
              allDates.push(r[k]);
            }
          }
        }
