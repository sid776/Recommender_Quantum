...
function pickKey(row, candidates) {
  ...
  // suffix match (e.g. "1.2 std_value" -> "std_value")
  for (const k of keys) {
    const nk = normKey(k);
...
function normalizeRows(rows) {
  return (rows || []).map((r) => {
    ...
    // Year columns (2021..2025 or Y2021..Y2025)
...
function buildColumnDefs(rows) {
  // Resolve one “best” physical key per preferred field (when consistent)
  ...
  const preferredCols = resolved.map(({ header, keys, actualKey }) => {
    ...
    if (actualKey) {
      ...
      // row-aware fallback to support "1.2 std_value"-style names
...
// ---------- export helpers ----------
function rowsForExport(rows, columnDefs) {
  // export only visible, non-group columns, in the same left→right order
  const cols = (columnDefs || [])
    .filter((c) => !c.rowGroup && (c.field || c.valueGetter))
    .map((c) => ({ header: c.headerName || c.field, field: c.field, valueGetter: c.valueGetter }));

  return {
    headers: cols.map((c) => c.header),
    records: (rows || []).map((r) =>
      cols.map((c) => (c.field ? r?.[c.field] : c.valueGetter?.({ data: r }) ?? null))
    ),
  };
}
...
function toCSV(data, sep = ",") {
  ...
  return [headers.map(esc).join(sep), ...records.map((row) => row.map(esc).join(sep))].join("\n");
}

function toJSONFile(data) {
  const { headers, records } = data;
  const objs = records.map((row) => {
    const o = {};
...
export default function CosmosReports() {
  ...
  const [exportFmt, setExportFmt] = useState("csv"); // csv | tsv | json
  ...
  const doExport = () => {
    const data = rowsForExport(normRows, columnDefs);
    const cob = reportDate || "latest";
    if (exportFmt === "csv") {
      download(`dq_reports_${cob}.csv`, "text/csv;charset=utf-8", toCSV(data, ","));
    } else if (exportFmt === "tsv") {
      download(`dq_reports_${cob}.tsv`, "text/tab-separated-values;charset=utf-8", toCSV(data, "\t"));
    } else {
      download(`dq_reports_${cob}.json`, "application/json;charset=utf-8", toJSONFile(data));
    }
  };
          ...
          <div className="p-4 bg-white shadow-md rounded-lg flex flex-col h-full">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-4">
                ...
                {/* Export controls (right side of header) */}
                <div className="flex items-center gap-2">
                  <Select size="sm" value={exportFmt} onChange={(e) => setExportFmt(e.target.value)} width="120px">
