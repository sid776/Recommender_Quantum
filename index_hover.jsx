...
function buildColumnDefs(rows, YEARS) {
  ...
  const clickableRenderer = (params) => {
    ...
    return (
      <span
        style={{
          cursor: looksNumeric ? "pointer" : "default",
          textDecoration: looksNumeric ? "underline" : "none",
...
export default function CosmosReports() {
  ...
  const onGridReady = (params) => {
    // show buttons but keep panels closed
  ...
  return (
    <Box className="overflow-hidden" height="calc(100vh - 70px)">
      {/* center the side buttons vertically (right side) */}
      <style>{`
