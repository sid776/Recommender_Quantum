...
/* util: find actual key in a row/rows from candidate list */
...
function getFromAny(row, keys) {
  ...
  // exact from candidates
  const key = pickKey(row, keys || []);
...
/* normalize rows (derive __book for grouping; map Y2021.. to 2021..) */
function normalizeRows(rows) {
  return (rows || []).map((r) => {
    ...
    // derive hidden book for grouping
    const bKey = pickKey(o, ["book", "book_nm", "book name"]);
    o.__book = bKey ? o[bKey] : "";

    // fill a date-ish field from report_date if present
...
/* build columns: grouped Report Type → Book; Years; then Preferred; then extras */
function buildColumnDefs(rows) {
  ...
  // Resolve each preferred column’s actual key using the dataset
  const resolvedPreferred = PREFERRED_FIELDS.map((pf) => ({
    ...pf,
    resolvedKey: findKeyInRows(rows, pf.keys),
  }));

  const preferredCols = resolvedPreferred
    .filter((pf) => !!pf.resolvedKey) // only show if we found a match in the data
    .map((pf) => ({
      headerName: pf.header,
      field: pf.resolvedKey,                 // bind to the actual key
      valueGetter: (p) => getFromAny(p?.data, [pf.resolvedKey, ...pf.keys]),
      sortable: true,
      resizable: true,
  ...
  // extras (anything not already shown)
  const seen = new Set([
    "report_type",
    "__book",
    "book",
    ...
    "report_date",
    ...FIXED_YEARS,
    ...resolvedPreferred.filter((p) => p.resolvedKey).map((p) => p.resolvedKey),
  ]);
  ...
  // order: groups → Report Date → Years → Preferred → Extras
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // default COB to the latest date in payload
          ...
          <div className="p-4 bg-white shadow-md rounded-lg flex flex-col h-full">
            ...
            >
              <AgGridReact
                ...
                /* Right-side tool panels: visible tabs, closed by default */
                sideBar={{
                  position: "right",
                  toolPanels: [
