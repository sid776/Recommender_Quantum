...
def _print_debug_object_source(obj_cls: type) -> None:
    ...
    # Many codebases set catalog via connection layer_map; if you know it (e.g. niwa_dev),
    # add it to the print below. We at least show schema.table so you can confirm in logs.
...
def _call_get_dataframe(obj_cls: type, report_date: Optional[date], limit: int):
    """
    Tries date-filtered call with the correct kwarg if supported; falls back to
    non-filtered call if empty; then increases limit to widen the slice.
    Returns (records, debug_trace).
    """
    ...
    # 3) Widen slice and try again (helps when latest rows are beyond small limit)
    big = max(limit, 2000)
    try:
        print(f"[DQ] Retrying {obj_cls.__name__}.get_dataframe(limit={big}, pyspark=False) for wider slice")
        df = fn(limit=big, pyspark=False)
        recs = _to_records(df)
        if report_date:
            recs = _filter_by_date(recs, report_date)
