...
// use the shared fetch helper (see Calculator)
...
// --- helpers to normalize date + find latest ---
function parseYMD(v) {
...
function maxDateStr(values) {
  ...
  for (const val of values || []) {
    ...
    if (!d) continue;
    if (!max || d > max) max = d; // lexicographic works for YYYY-MM-DD
  }
  return max;
}
// ------------------------------------------------
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // use the shared hook (returns parsed JSON)
      const json = await useFetch(url);
      const data = Array.isArray(json) ? json : Array.isArray(json?.rows) ? json.rows : [];
      setRows(data || []);

      // auto-pick latest date **only** on initial load (no date chosen)
      if ((!dateStr || !dateStr.length) && data?.length) {
        const latest = maxDateStr(data.map((r) => r?.report_date));
