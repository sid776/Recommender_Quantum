...
export default function CosmosReports() {
  ...
  async function loadData() {
    try {
      ...
      const dateParam =
        safeToYYYYMMDD(reportDate) || (typeof reportDate === "string" ? reportDate : "");
      if (!name || !dateParam) {
        setRows([]);
        return;
      }
      const base = window.location.origin;
      const url = `${base}/api/dq/${encodeURIComponent(name)}?report_date=${dateParam}&limit=500`;
      console.log("GET", url);
