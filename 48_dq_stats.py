...
def _latest_date_across_objects() -> Optional[date]:
    ...
    for _, cls in SECTIONS:
        ...
        # try common ordering patterns first (fast path)
        for kwargs in (dict(limit=1, pyspark=False, order=['REPORT_DATE__DESC']),
                       dict(limit=1, pyspark=False, order=['report_date__desc'.upper()])):
            try:
                d = _max_date_in_records(_df_to_records(fn(**kwargs)))
                if d and (latest is None or d > latest):
                    latest = d
            except:
                pass
        # then a slightly wider, unordered probe
        if latest is None:
            try:
                d = _max_date_in_records(_df_to_records(fn(limit=500, pyspark=False)))
                if d and (latest is None or d > latest):
                    latest = d
            except:
                pass
    ...
    if report_date is not None:
        for name, cls in SECTIONS:
            try:
                ...
                pass
    # still return something if no date or no matches
    if not rows:
        for name, cls in SECTIONS:
            try:
