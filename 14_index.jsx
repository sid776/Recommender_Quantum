...
// NEW: search a key across ALL rows
...
function buildColumnDefs(rows, YEARS) {
  ...
  // detect detail keys across ALL rows (not just the first)
  ...
  // build extras from union of keys across ALL rows
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      setLoading(false);
      // Auto-size after data load to expose all columns nicely
  ...
  // Keep side buttons visible, but panel closed on load
  const onGridReady = (params) => {
    params.api.setSideBarVisible(true); // show the buttons (Columns / Filters)
    params.api.closeToolPanel();        // keep the panel closed by default
  };
          ...
          <div className="p-4 bg-white shadow-md rounded-lg flex flex-col h-full">
            ...
            >
              <AgGridReact
                ...
                /* Columns/Filters buttons on the right; panel closed by default */
                sideBar={{
                  position: "right",
