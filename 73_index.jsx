...
/* Normalize for fuzzy key matching */
const norm = (s) =>
...
const PREFERRED_FIELDS = [
  ...
  {
    header: "Risk Factor Id",
    keys: ["risk_factor_id", "risk factor id", "rf_id", "riskfactorid"],
  },
  {
    header: "Stale Date",
    keys: [
  ...
  {
    header: "Std Value",
    keys: ["std_value", "std value", "std", "stddev", "std_dev"],
  },
  {
    header: "Mean Value",
    keys: ["mean_value", "mean value", "mean"],
  },
...
function normalizeRows(rows) {
  return (rows || []).map((r) => {
    ...
    // fill stale_date from report_date if missing
    ...
    // numeric coercion only if value exists
    ["mean_value", "z_score", "std_value"].forEach((k) => {
...
function buildColumnDefs(rows) {
  ...
  const preferredCols = preferredKeyMap.map(({ header, actualKey, keys }) => {
    const isNumeric = keys.some((k) => numericKeySet.has(norm(k)));
    const col = {
      headerName: header,
      sortable: true,
      resizable: true,
            ...
            >
              <AgGridReact
                ...
                autoGroupColumnDef={{
                  ...
                  valueGetter: (p) =>
                    p?.node?.key
                      ? `${p.node.key} (${p.node.allLeafChildren?.length || 0})`
                      : "",
                }}
                headerHeight={42}
