...
function buildColumnDefs(rows, YEARS) {
  ...
  return allKeys.map((k) => {
    ...
    // >>> minimal change: bind detail columns directly and aggregate with 'first'
    if (isDetail) {
      col.field = k;                 // ensure binding to real field
      col.aggFunc = "first";         // show something on grouped rows
      col.suppressAggFuncInHeader = true;
      // removed the previous valueGetter/group nulling
    }
    // <<<
