...
/* robust date parsing (YYYY-MM-DD, YYYYMMDD, ISO, YYYY/M/D, YYYY-M-D) */
function parseYMD(anyVal) {
  ...
  let s = String(anyVal).trim().replace(/['"]/g, "");
  if (/^\d{4}-\d{1,2}-\d{1,2}([ T].*)?$/.test(s)) s = s.slice(0, 10);
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // *** Minimal, deterministic default-to-latest logic ***
      if ((!dateStr || !dateStr.length) && data?.length) {
        const priority = ["report_date", "as_of_date", "as_of_dt", "cob_date"];
        let latest = null;
        for (const key of priority) {
          const vals = data.map((r) => r?.[key]).filter((v) => v != null);
          const cand = maxDateStr(vals);
          if (cand) { latest = cand; break; }
        }
        if (latest) {
  ...
  const onGridReady = (params) => {
    ...
    params.api.closeToolPanel(); // keep panel closed by default
  };
