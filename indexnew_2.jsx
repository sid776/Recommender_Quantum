...
export default function CosmosReports() {
  ...
  async function onRun() {
    setErrorText("");
    const d = getValues("report_date"); // HTML date input -> yyyy-mm-dd
    setActiveRule(null);
    setActiveBook(null);
    setRuleOpenMap({});
    if (!d) {
    ...
    setLoading(true);
    try {
      const url = `${API_ENDPOINT}?report_date=${encodeURIComponent(d)}&limit=500`;
      const res = await fetch(url, { headers: { Accept: "application/json" } });

      // if backend crashed, body will be HTML/text (Traceback...), not JSON
      const ct = res.headers.get("content-type") || "";
      if (!res.ok) {
        const txt = await res.text();
        setErrorText(`Server error ${res.status}. ${txt.slice(0, 400)}`);
        setRows([]);
        return;
      }
                            ...
                            <div key={rt} className="border rounded-md bg-gray-50">
                              <div className="flex items-center justify-between px-3 py-2">
                                ...
                                >
                                  ...
                                  <span
                                    className={`${
                                      String(activeRule) === rt && !activeBook ? "font-semibold" : ""
                                    }`}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setActiveRule((prev) =>
                              ...
                              {open && (
                                <div className="flex flex-col pb-2">
                                  {booksForRule.map((bk) => {
                                    ...
                                    const selected =
                                      String(activeRule) === rt && String(activeBook) === bk;
                                    return (
                                      <div
                                        key={`${rt}||${bk}`}
