...
/* Normalize rows: ensure 2021..2025 exist; map Y2021..Y2025 → 2021..2025; tidy types */
function normalizeRows(rows) {
  return (rows || []).map((r) => {
    ...
    // Populate a “stale-ish” date if empty using report_date
...
/* Columns:
   - Always includes preferred columns (via valueGetter with aliases)
   - report_type is used for grouping (hidden)
   - Fixed year columns 2021..2025
   - Any extras after that
*/
function buildColumnDefs(rows) {
  // Preferred columns always present
  const preferredCols = PREFERRED_FIELDS.map((pf) => ({
    headerName: pf.header,
    field: pf.keys[0], // dummy field to satisfy grid
  ...
  // Extras (anything not already covered, excluding Y2021..)
  const seen = new Set([
    "report_type",
    "report_date",
  ...
  return [...baseCols, ...preferredCols, ...yearCols, ...extras];
}
...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // Default COB to latest present in payload
