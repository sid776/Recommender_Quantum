...
function getYearKeys(rows) {
  ...
  const years = Array.from(
    new Set(
      rows.flatMap((r) => Object.keys(r || {}).filter((k) => /^\d{4}$/.test(k) && Number(k) >= MIN_YEAR))
    )
  ).sort();
...
export default function CosmosReports() {
  ...
  // 🔧 Robust fetch shim: supports multiple `useFetch` shapes and falls back to window.fetch
  const fetchApi = useFetch?.(); // could be a function or an object
  async function httpGet(url) {
    try {
      if (fetchApi?.get) return await fetchApi.get(url);        // { get } style
      if (typeof fetchApi === "function") return await fetchApi(url); // function style
      const res = await fetch(url, { headers: { Accept: "application/json" } }); // fallback
