...
// -------- minimal helpers for “latest date on load” ----------
function fmt(d) {
...
async function tryFetchForDate(dateStr) {
  const url = `${API_ENDPOINT}?report_date=${encodeURIComponent(dateStr)}&limit=500`;
...
export default function CosmosReports() {
  ...
  // NEW: on first load, pick the most recent available date by probing backwards from today.
  useEffect(() => {
    ...
    (async () => {
      ...
      for (let i = 0; i < 21; i++) { // look back up to 3 weeks
        const ymd = fmt(d);
        const data = await tryFetchForDate(ymd);
      ...
      // If still nothing, just call the generic endpoint to show *something*
      if (!picked) {
        await fetchData("");
        return;
      }

      // Set the COB input without marking the form dirty
      setValue("report_date", picked, { shouldDirty: false });
    })();
  }, [setValue]);
