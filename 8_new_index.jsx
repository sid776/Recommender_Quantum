...
export default function CosmosReports() {
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // Manager note: "conversion is not required"
      // Keep whatever date server returns; don’t massage it here.
      if ((!dateStr || !dateStr.length) && data?.length) {
        const rd = data[0]?.report_date;
        if (rd) setValue("report_date", rd, { shouldDirty: false });
      }
    } finally {
      setLoading(false);
    ...
    <Box className="overflow-hidden" height="calc(100vh - 70px)">
      <style>{`
        .custom-grid .ag-side-buttons {
          ...
          margin-top: 24px; /* nudge down a bit */
        }
      `}</style>
          ...
          <div className="p-4 bg-white shadow-md rounded-lg flex flex-col h-full">
            ...
            >
              <AgGridReact
                ...
                sideBar={{
                  position: "right",
                  hiddenByDefault: false, // show the buttons; panel stays closed
                  toolPanels: [
