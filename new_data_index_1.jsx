...
function normalizeRows(rows) {
  ...
  return (rows || []).map((r) => {
    ...
    // Ensure all year columns exist and are numeric (default 0)
    YEARS.forEach((y) => {
      const v = o[y];
      const n = Number(v);
...
function buildColumnDefs(rows) {
  ...
  const allKeys = [...ordered, ...extras]; // do NOT filter/hide any columns
  ...
  return allKeys.map((k) => {
    ...
    if (k === "rule_type" || k === "book") {
      col.rowGroup = true;
      col.hide = true; // keep grouping by rule/book like before
    }
    if (isYear) {
