...
export default function CosmosReports() {
  const methods = useForm({
    defaultValues: {
      // 5) default to DQ Availability
      reportName: REPORTS.find((r) => r.value === "availability") || REPORTS[0],
  ...
  return (
    <FormProvider {...methods}>
      <Box className="mx-auto max-w-[1400px] space-y-4 p-4" style={{ overflow: "visible" }}>
        {/* Toolbar directly above the grid (4: “Reports” on top of AG Grid) */}
        <Box className="bg-white rounded-lg shadow-lg p-4" style={{ position: "relative", zIndex: 9999, overflow: "visible" }}>
          <HStack justify="space-between" align="center" mb={2}>
            <Text fontSize="lg" fontWeight="bold">{/* no big report title; toolbar matches calculator style */}</Text>
            <HStack spacing={3}>
              <Button
                size="sm"
                variant="outline"
          ...
          {/* 2) Align fields like existing pages; 7) clearing via X should clear everything */}
          <Wrap align="center" spacing="24px" style={{ overflow: "visible" }}>
            <WrapItem style={{ minWidth: 320, position: "relative", zIndex: 10000, overflow: "visible" }}>
              <DynamicSelect
                ...
                defaultValue={methods.getValues("reportName")}
              />
            </WrapItem>
          ...
          {/* Spacer pushes grid down while dropdown menu is open/focused so it never hides behind */}
          <Box
        ...
        {/* Data grid */}
        <Box className="bg-white rounded-lg shadow-lg p-2" style={{ height: "calc(100vh - 260px)", position: "relative", zIndex: 1, overflow: "visible" }}>
          {loading ? (
