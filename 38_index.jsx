...
// no direct AgGridReact import; use the shared wrapper instead
// no extra libs added
...
export default function CosmosReports() {
  ...
  const fetchJson = useFetch(); // shared helper
  ...
  async function fetchData(dateStr) {
    ...
    try {
      ...
      // Per review: conversion not required — use the first row’s report_date directly (ISO slice)
      if ((!dateStr || !dateStr.length) && data?.length) {
        const first = data[0]?.report_date ?? "";
        const iso = typeof first === "string" ? first.slice(0, 10) : "";
        if (iso) setValue("report_date", iso, { shouldDirty: false });
      }
    } finally {
      setLoading(false);
